...
def sync_check(data, group, parallel_mode):
    """Check whether the data is the same across all ranks in the group in current parallel mode."""
    world_size = gpc.get_world_size(parallel_mode)
    data_send = data.clone().detach().to(device=torch.cuda.current_device())
    data_recv_list = [torch.zeros_like(data, device=torch.cuda.current_device()) for _ in range(world_size)]
    dist.all_gather(data_recv_list, data_send, group)

    for idx, data_recv in enumerate(data_recv_list):
        assert torch.all(data_send == data_recv), f"local {idx} and global {gpc.get_global_rank()} data mismatch."
...
def main(args):
    ...
    data_check_counts = gpc.config.get("data_check_counts", 2)
    ckpt_manager.try_save_checkpoint(train_state, force=True)
    save_tgt = gpc.config.SAVED_DATA_PATH
    current_file = None
    ...
    with initialize_llm_profile(profiling=args.profiling, start_time=current_time) as prof:
        ...
        for batch_count in range(train_state.batch_count, total_steps):
            ...
            # load batch data
            if current_file != f"step-{(batch_count // 1000 + 1) * 1000}":
                batch_buffer = torch.load(
                    os.path.join(
                        save_tgt,
                        f"step-{(batch_count // 1000 + 1) * 1000}/rank-{gpc.get_local_rank(ParallelMode.DATA)}.pt",
                    )
                )
                current_file = f"step-{(batch_count // 1000 + 1) * 1000}"
            batch = batch_buffer[batch_count % 1000]

            if data_check_counts > 0:
                if gpc.is_initialized(ParallelMode.MODEL) and gpc.get_world_size(ParallelMode.MODEL) > 1:
                    group = gpc.get_group(ParallelMode.MODEL)
                    inputs, output_label = batch
                    inputs_keys = sorted(list(inputs.keys()))
                    for inputs_key in inputs_keys:
                        if isinstance(inputs[inputs_key], list):
                            for data in inputs[inputs_key]:
                                sync_check(data, group, ParallelMode.MODEL)
                        else:
                            sync_check(inputs[inputs_key], group, ParallelMode.MODEL)
                    sync_check(output_label, group, ParallelMode.MODEL)
                data_check_counts -= 1
